...
def get_water_level_fromurl(river_name):
    water=0
    uroven=0
    res={}
    rivercode=0
    ...
    for rivobj in all_riverobj:
        if rivobj.name == river_name:
            ...
            if getdatafromhtml(rivobj.data_url) !=None:
                mrlist=getdatafromhtml(rivobj.data_url)
                water=float(mrlist.pop(2))
                value_bc=float(rivobj.uroven)+water/100

                water = value_bc
                return water
            else:
                water=float(rivobj.uroven)
                return water
...
def main(request):
    ...
    if 'river_list' in request.POST:
        ...
        if ping("pinhmc.pogoda.by:1433")==True:
            ...
            return render(request,"prognoz.html",{'rivers':riverobj,'data_aval':data_aval,'inpdata':input_data,'selvalue':selected_value,'water_lev':water_level,'nameags':name_ags}, )
        else:
            water_level=get_water_level_fromurl(selected_value)
            name_ags=get_name_ags(selected_value)
            data_aval=get_data_avaliability(selected_value)
            return render(request,"prognoz.html",{'rivers':riverobj,'data_aval':data_aval,'inpdata':input_data,'selvalue':selected_value,'water_lev':water_level,'nameags':name_ags}, )

    return render(request,"prognoz.html",{'rivers':riverobj,'data_aval':data_aval,'inpdata':input_data,'water_lev':water_level,'nameags':name_ags}, )
...
def results(request):
    ...
    if 'river_list' in request.POST:
        ...
        return render(request, 'results.html',{'dtchart':cht1,'rivers':riverobj,'selvalue':selected_value})
    else:
        selected_value=''
        print 'out'
...
def get_level_set(map_id,distance):

    dist_in=distance-1
    dist_out=distance+1
    prognoz_obj_first=Prognozdata.objects.order_by('-distance_float').filter(map=map_id,distance__range=(dist_in,dist_out)).first()
    prognoz_obj_last=Prognozdata.objects.order_by('-distance_float').filter(map=map_id,distance__range=(dist_in,dist_out)).last()

    if prognoz_obj_first!=None:
        dist_start=prognoz_obj_first.distance_float
        level_start=prognoz_obj_first.level
        if prognoz_obj_last!=None:
            dist_last=prognoz_obj_last.distance_float
            level_last=prognoz_obj_last.level

            calclevel=float(level_start)+((float(level_last)-float(level_start))/(float(dist_last)-float(dist_start)))*(float(distance)-float(dist_start))
...
def post_form_upload(request):
    ...
    if request.method == 'GET':
       ...
       # Get array to PrognozData
        ...
        cht1 = Chart(
            ...
            chart_options=
               {'chart':
                ...
                'colors': ['#4ee15e','#058DC7','#FFF263', '#ED561B', '#8085e9',
                '#f15c80', '#e4d354', '#2b908f', '#f45b5b', '#91e8e1'],

                  'title':
                    {
                    'text': 'Прогнозный уровень воды, м относительно бровки  ' + ' || станция AГС: ' + selected_ags.encode('utf-8')+ ',река: '+selected_river.encode('utf-8')
                    },
                 ...
                 'yAxis':
                    ...
                    [{
                      ...
                      },
                      {
                       ...
                       'minorTickWidth':1,
                       'opposite': False,
                       'title':  {'text': ' Уровень относительно бровки, м '},
                       'gridZIndex': 4,
                       'ceiling' : 3.0,
                       'max': 3.0,
        ...
        rural=Settlements.objects.all().filter(idriver=river_obj.river_code,start__lte=max_dist,end__gte=min_dist)
        # get Settlements where  start max_dist   ----> end min_dist

        rur_flood = FloodSet.objects.all()
        if rur_flood.count()>0:
            FloodSet.objects.all().delete()
        ...
        for rural_obj in rural:
            ...
            level_start=get_level_set(map_id,dist_rural_first)
            level_end = get_level_set(map_id,dist_rural_end)

            if level_start<> None:
                var1=rural_obj.start_alarm-Decimal(level_start)
            else:
                var1=rural_obj.start_alarm

            if level_end <> None:
                var2=rural_obj.end_alarm-Decimal(level_end)
            else:
                var2=rural_obj.end_alarm

            if var1>var2:
                newobj=FloodSet()
                newobj.dist_km=int(dist_rural_first)
                newobj.distance=float(dist_rural_first)
                newobj.level=level_start
                newobj.level_danger=rural_obj.start_alarm
                newobj.name=name_rural
                newobj.save()
            if var2>=var1:
                newobj=FloodSet()
                newobj.dist_km=int(dist_rural_end)
                newobj.distance=float(dist_rural_end)
                newobj.level=level_end
                newobj.level_danger=rural_obj.end_alarm
                newobj.name=name_rural
                newobj.save()
        ...
        return render(request, 'calc.html', {'dtchart':[cht3,cht1],'set_spot':set_spot,'form': form ,'levags':selected_value, 'river': selected_river , 'ags': selected_ags  },)
